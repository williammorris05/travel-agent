...
Each planning request consumes at most one hotel call. Attempts—including errors—count against a process-local allowance of at most 10. There are no retries, pagination, property-detail calls or background fetches. Slider PATCHes make no provider calls. Chat can discuss existing session results for up to five minutes when dates/adults match, preserving their original retrieval timestamp. Explicit refreshes fetch again with provider caching disabled. Calls are limited to 20 seconds overall. The counter resets on restart and is not an account-wide spending ledger: keep the account on the free plan and revisit persistent accounting before deployment. Current offers remain in the ephemeral session; there is no cross-session provider cache or disk content archive.

Provider failures return 503 and retain previous results. Revision conflicts reject late results. Dates/adults are sent with each search and checked against returned search parameters. Only numeric whole-stay totals with usable HTTPS property links are displayed; nightly minima are never multiplied into invented totals. Links may not reproduce the displayed search rate. Confirm final prices on the property/provider site.
